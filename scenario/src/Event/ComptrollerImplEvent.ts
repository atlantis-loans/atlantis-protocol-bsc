...
async function genComptrollerImpl(world: World, from: string, params: Event): Promise<World> {
  let { world: nextWorld, comptrollerImpl, comptrollerImplData } = await buildAtlantistrollerImpl(
    world,
    from,
    params
  );
...
async function becomeG3(
  ...
  let invokation = await invoke(
    world,
    comptrollerImpl.methods._become(unitroller._address, atlantisRate, atlantisMarkets, otherMarkets),
    from,
    ComptrollerErrorReporter
  );
...
export function comptrollerImplCommands() {
  return [
    ...
    new Command<{
      unitroller: Unitroller;
      comptrollerImpl: ComptrollerImpl;
      atlantisRate: NumberV;
      atlantisMarkets: ArrayV<AddressV>;
      otherMarkets: ArrayV<AddressV>;
    }>(
      `
        #### BecomeG3

        * "ComptrollerImpl <Impl> BecomeG3 <Rate> <AtlantisMarkets> <OtherMarkets>" - Become the comptroller, if possible.
          * E.g. "ComptrollerImpl MyImpl BecomeG3 0.1e18 [aDAI, aBNB, aUSDC]
      `,
      'BecomeG3',
      [
        new Arg('unitroller', getUnitroller, { implicit: true }),
        new Arg('comptrollerImpl', getComptrollerImpl),
        new Arg('atlantisRate', getNumberV, { default: new NumberV(1e18) }),
        new Arg('atlantisMarkets', getArrayV(getAddressV),  {default: new ArrayV([]) }),
        new Arg('otherMarkets', getArrayV(getAddressV), { default: new ArrayV([]) })
      ],
      (world, from, { unitroller, comptrollerImpl, atlantisRate, atlantisMarkets, otherMarkets }) => {
        return becomeG3(world, from, comptrollerImpl, unitroller, atlantisRate.encode(), atlantisMarkets.val.map(a => a.val), otherMarkets.val.map(a => a.val))
      },
      { namePos: 1 }
    ),
    ...
    new Command<{
      ...
      `
        ...
        * "ComptrollerImpl <Impl> Become <Rate> <AtlantisMarkets> <OtherMarkets>" - Become the comptroller, if possible.
          * E.g. "ComptrollerImpl MyImpl Become 0.1e18 [aDAI, aBNB, aUSDC]
      `,
      'Become',
      [
        new Arg('unitroller', getUnitroller, { implicit: true }),
