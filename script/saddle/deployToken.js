...
note: pass VERIFY=true and BNBERSCAN_API_KEY=<api key> to verify contract on Etherscan
...
(async function() {
  ...
  let contract = await saddle.deploy('ABep20Delegator', deployArgs);

  console.log(`Deployed contract to ${contract._address}`);

  if (env['VERIFY']) {
    const etherscanApiKey = env['BNBERSCAN_API_KEY'];
    if (etherscanApiKey === undefined || etherscanApiKey.length === 0) {
      throw new Error(`BNBERSCAN_API_KEY must be set if using VERIFY flag...`);
    }
    ...
    await saddle.verify(etherscanApiKey, contract._address, 'ABep20Delegator', deployArgs, 0);
