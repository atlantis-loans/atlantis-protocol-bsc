...
note: $BNBERSCAN_API_KEY environment variable must be set to an Etherscan API Key.
...
(async function() {
  ...
  let etherscanApiKey = env['BNBERSCAN_API_KEY'];
  if (!etherscanApiKey) {
    console.error("Missing required $BNBERSCAN_API_KEY env variable.");
    return printUsage();
  }
  ...
  await saddle.verify(etherscanApiKey, address, 'ABep20Immutable', deployArgs, 200, undefined);
