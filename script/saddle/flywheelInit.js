...
usage:
	npx saddle script -n {network} flywheel:init {
		batch: number || null, // how many borrowrs to claim atlantis for at once. default 100 (~2.3M gas)
...
let filterInitialized = async (borrowersByAToken) => {
	let res = {}
	let batchSize = 75;
	console.log(`Calling atlantisBorrowerIndex for borrowers in batches of ${batchSize}...\n`);
	for(let aTokenAddr of Object.keys(borrowersByAToken)) {
		let supplySpeed = await call(Comptroller, 'atlantisSupplySpeeds', [aTokenAddr]);
		let borrowSpeed = await call(Comptroller, 'atlantisBorrowSpeeds', [aTokenAddr]);
			...
			for (let borrowerChunk of getChunks(borrowersByAToken[aTokenAddr], batchSize)) {
				try {
					let indices = await Promise.all(borrowerChunk.map(
						async(borrower) => {
							return await call(Comptroller, 'atlantisBorrowerIndex',[aTokenAddr, borrower])
					}));
...
let claimAtlantisBatch = async (borrowersByAToken, opts) => {
	for (let aTokenAddr of Object.keys(borrowersByAToken)) {
		...
		for (let chunk of getChunks(borrowers, opts.batch)) {
			if (chunk.length == 0) {
				...
				try {
					let tx = await send(Comptroller, 'claimAtlantis', [
						chunk,
						[aTokenAddr],
						true,
						false,
...
(async () => {
	...
	await claimAtlantisBatch(unInit, opts);
